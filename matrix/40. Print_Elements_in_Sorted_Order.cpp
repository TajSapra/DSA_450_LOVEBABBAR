...
void fun(){
    ...
    // approach I will use: Make a vector, push all elements from 2d array into it and sort the vector. 
    // Time complexity: O(n*n*log(n*n)) which is O(n^2*log(n)) as log(n*n) = 2*log(n) and 2 can be ignored. So overall time complexity is O(n^2*log(n))
    // Space complexity: O(n*n)
    vi elements;
    fr(i,0,n,1){
        fr(j,0,m,1){
            elements.pb(input[i][j]);
        }
    }
    sort(all(elements));
    fr(i,0,elements.size(),1){
        cout<<elements[i]<<" ";
    }    
}
int main(){

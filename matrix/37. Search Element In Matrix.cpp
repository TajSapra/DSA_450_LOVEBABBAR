...
void fun(){
    ...
    // accoring to the problem, the matrix given to us is sorted row wise. Meaning that every row of the matrix is in sorted manner and the first element of the next row is greater than or equal to the last element of the current row.
    fr(i,0,n,1){
        sort(all(input[i]));
    }
    // we have to start writing the code after that so the complexity of sorting the row vectors is not to be included.
    int key;
    cin>>key;
    for(int i=0;i<n-1;i++){
        if(input[i][0]<key&&input[i+1][0]>=key){
            cout<<binary_search(all(input[i]),key);
            return;
        }
    }
    cout<<binary_search(all(input[n-1]),key);
}
int main(){
    int t=1;
    // cin>>t;

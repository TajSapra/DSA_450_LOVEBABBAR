...
void fun(){
    ...
    sort(all(input));
    int maxi=INT_MIN, mini=INT_MAX, ans=input[n-1]-input[0];
    fr(i,1,n,1){
        if(input[i]>=k){
            // compute the maximu andd minimum element at every pass and checking their difference
            maxi=max(input[i-1]+k, input[n-1]-k);
            mini=min(input[i]-k, input[0]+k);
